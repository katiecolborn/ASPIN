...
Generate_statistics_res_SYSEP <- function(nfolds=10,train_date_end="2017-10-01",column_sums=c(5,10,15,20),fdr_col=c(0,1,0.12,0.15,0.2),offset=1,parallelcores ="max",data_type="Rdata",
                                    data_path="//data.ucdenver.pvt/dept/SOM/ACCORDS/PiFolders/PI_Colborn/UCHealthSOR/Data/Data_20210113/final_analytical_datasets_0405/SYSEP/",
                                    fileName ="final_data_labpanel_SYSEP",save_res=TRUE,save_dir="C:/Users/ASRock/OneDrive - The University of Colorado Denver/Katie Project/2021_0425/SYSEP/res_0429/")
{
  ...
  AMA_path <- substr(data_path,1,nchar(data_path)-51)
  ...
  for (i in 1:length(column_sums)) {
    ...
    #nfolds <- 10
    set.seed(1234)
    W = stat.glmnet_coefdiff(l, X_k, y=training_set$othsysep, nfolds=nfolds, family="binomial",cores = numCores)
    #hist(W,breaks = 100)
    #hist(W)
    Sys.sleep(0.1)
    # update progress bar
    ...
    for (j in 1:length(fdr_col)) {
      ...
      train2 <- training_set[c("othsysep", names(m))]
      glm1 <- glm(othsysep ~ ., data = train2, family="binomial")
      ...
      cmFull.1 <- as.data.frame(cbind(training_set$othsysep, fit1))
      colnames(cmFull.1) <- c("othsysep", "PredSYSEP")
      
      ### Training set fit
      
      roc_lasso1.1 <- roc(cmFull.1$othsysep, cmFull.1$PredSYSEP)
      #roc_lasso1.1
      ...
      cmFull.2 <- as.data.frame(cbind(testing_set$othsysep, fit2))
      colnames(cmFull.2) <- c("othsysep", "PredSYSEP")
      
      ### test set fit
      roc_lasso1.2 <- roc(cmFull.2$othsysep, cmFull.2$PredSYSEP)
      #roc_lasso1.2
