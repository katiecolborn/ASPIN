...
Generate_statistics_res_Pneu <- function(nfolds=10,train_date_end="2017-10-01",column_sums=c(5,10,15,20),fdr_col=c(0,1,0.12,0.15,0.2),offset=1,parallelcores ="max",data_type="Rdata",
                                    data_path="//data.ucdenver.pvt/dept/SOM/ACCORDS/PiFolders/PI_Colborn/UCHealthSOR/Data/Data_20210113/final_analytical_datasets_0405/PNEU/",
                                    fileName ="final_data_labpanel_SSIUTIPNEU",save_res=TRUE,save_dir="C:/Users/ASRock/OneDrive - The University of Colorado Denver/Katie Project/2021_0425/Pneumonia/res_0429/")
{
  ...
  AMA_path <- substr(data_path,1,nchar(data_path)-50)
  ...
  X <- data.matrix(training_set[, c(315:(ncol(training_set)-2))]) 
  #dim(X)
  ...
  for (i in 1:length(column_sums)) {
    ...
    #nfolds <- 10
    set.seed(1234)
    W = stat.glmnet_coefdiff(l, X_k, y=training_set$oupneumo, nfolds=nfolds, family="binomial",cores = numCores)
    #hist(W,breaks = 100)
    #hist(W)
    Sys.sleep(0.1)
    # update progress bar
    ...
    for (j in 1:length(fdr_col)) {
      ...
      train2 <- training_set[c("oupneumo", names(m))]
      glm1 <- glm(oupneumo ~ ., data = train2, family="binomial")
      ...
      cmFull.1 <- as.data.frame(cbind(training_set$oupneumo, fit1))
      colnames(cmFull.1) <- c("oupneumo", "PredPneu")
      
      ### Training set fit
      
      roc_lasso1.1 <- roc(cmFull.1$oupneumo, cmFull.1$PredPneu)
      #roc_lasso1.1
      ...
      cmFull.2 <- as.data.frame(cbind(testing_set$oupneumo, fit2))
      colnames(cmFull.2) <- c("oupneumo", "PredPneu")
      
      ### test set fit
      roc_lasso1.2 <- roc(cmFull.2$oupneumo, cmFull.2$PredPneu)
      #roc_lasso1.2
